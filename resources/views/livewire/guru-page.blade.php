...
<div>
        ...
        <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            @foreach ($kepseks as $kepsek)
                <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-8">
                    ...
                    <div class="mt-8 md:mt-0 relative bg-[#C4DAD2] rounded-lg shadow-lg w-auto transition-transform transform hover:translate-y-2 hover:-translate-x-2 hover:scale-105 hover:shadow-2xl"
    ...
    <!-- Team -->
    <section>
        <div class="max-w-full px-4 py-5 sm:px-6 lg:px-8 lg:py-17 mx-auto">
            <div class="max-w-2xl mx-auto text-center mb-5 lg:mb-7" data-aos="fade-up">
            ...
            <div class="mt-8 flex justify-center">
                <ul id="pagination-links" class="inline-flex items-center space-x-1">
                    ...
                    @else
                        <li>
                            <a href="{{ $pengajars->previousPageUrl() }}"
                                class="ajax-pagination px-3 py-1 rounded-md bg-[#6A9C89] text-white hover:bg-[#5e8b7d]">
                                Previous</a>
                        </li>
                    @endif
                    ...
                    @foreach ($pengajars->getUrlRange(1, $pengajars->lastPage()) as $page => $url)
                        <li>
                            <a href="{{ $url }}"
                                class="ajax-pagination {{ $page == $pengajars->currentPage() ? 'bg-[#16423C] text-white' : 'bg-[#6A9C89] text-white' }} px-3 py-1 rounded-md hover:bg-[#5e8b7d]">
                                {{ $page }}
                            </a>
                        </li>
                    @endforeach
                    ...
                    @if ($pengajars->hasMorePages())
                        <li>
                            <a href="{{ $pengajars->nextPageUrl() }}"
                                class="ajax-pagination px-3 py-1 rounded-md bg-[#6A9C89] text-white hover:bg-[#5e8b7d]">Next</a>
                        </li>
                    @else
